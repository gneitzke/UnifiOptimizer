...
def test_4hour_bucketing(history):
    ...
    bucket_size = 4
    buckets = defaultdict(
        lambda: {"rx_dropped": [], "tx_dropped": [], "total_dropped": [], "timestamps": []}
    )
    ...
    for bucket_key in sorted(buckets.keys()):
        ...
        rx_dropped_data.append(
            sum(bucket["rx_dropped"]) / len(bucket["rx_dropped"]) if bucket["rx_dropped"] else 0
        )
        tx_dropped_data.append(
            sum(bucket["tx_dropped"]) / len(bucket["tx_dropped"]) if bucket["tx_dropped"] else 0
        )
    ...
    for i in range(min(5, len(labels))):
        print(
            f"  {labels[i]}: Total={total_dropped_data[i]:.1f}, RX={rx_dropped_data[i]:.1f}, TX={tx_dropped_data[i]:.1f}"
        )
