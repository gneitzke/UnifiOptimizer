...
| `wired.broadcast_storm` | broadcast/multicast pps > 10× 24 h baseline on multiple ports at once; ports whose link is down are skipped (combo uplinks mirror one uplink's counters onto both entries) | P1 |
...
| `wifi.sticky_client` | RSSI < −75 sustained ≥ 10 min while another AP measured ≥ 8 dB better **for this client, over that AP's own attachment intervals** (its `ap_mac` trail joined to its RSSI samples, ≥ 3 readings, 7-day lookback — the same horizon `net.coverage_hole` uses, so no client falls between the two); corroborate low rates + high retries. One issue per client: the AP it is glued to is evidence, not identity. Confounders: no better AP = coverage hole, a different issue; a candidate AP with a saturated radio or a weak mesh backhaul is not "better" and is never offered | P3, P2 if clustered on one AP |
...
- **Recurrence**: an intermittent condition oscillates between `active` and `resolving` indefinitely, because each refire resets `clear_streak` and K=6 is never reached. That is the state machine working; what was missing was any way to see it, since an issue on its ninety-eighth bounce rendered identically to one clearing for the first time. Recurrence is therefore a **derived presentation label, not a state**: `Repository.issue_streak_reset_counts` counts the trail rows that record a killed streak — `reopened`, plus `escalated` with `reason: refire_during_resolving` — over the last 7 days, and `GET /api/issues` (and `/{id}`) attach `lifecycle: {clear_k, streak_resets_7d, recurring}`, `recurring` at ≥ 2 resets. `clear_k` comes from the live `EngineConfig.k_for`, so a per-detector `detector_k` override is respected and the list can show "3 of 6 clean checks" instead of a bare spinner. `occurrences` is deliberately **not** the flapping signal: it increments on every fire cycle, so a steadily-burning issue racks it up without ever flapping. A new `IssueState` was rejected — it would ripple through the open-issue unique index semantics, `list_issues(state=)`, `correlatable_issues`, inhibition's cause-state check, the web union and every WS/HA consumer, to encode one bit the event trail already carries exactly; and a derived label renders correctly for historical rows the moment it ships, with no backfill (Gitea #39).
