...
def main(host, username, password, site="default"):
    ...
    console.print(Panel(f"[bold]CloudKey Authentication Test Suite[/bold]\nHost: {host}\nSite: {site}",
                  title="Test Runner", border_style="cyan"))
    
    # List of test scripts to run
    test_scripts = [
        {"name": "CloudKey Gen2+ API Client Test", "script": "tests/cloudkey/test_cloudkey_gen2_client.py"}
    ]
    ...
    for test in test_scripts:
        ...
        if not success:
            if not Confirm.ask(f"[yellow]Test {test_name} failed. Continue with next test?[/yellow]", default=True):
    ...
    if all_success:
        ...
        console.print("[green]Your CSRF token manager implementation is working correctly with CloudKey Gen2 devices.[/green]")
    else:
