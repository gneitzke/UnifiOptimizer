...
MANUFACTURER_PATTERNS = {
    ...
    'Google': {'type': 'iot', 'concern': 'Smart home device - check for firmware updates', 'icon': '🏠'},
    'Apple': {'type': 'consumer', 'concern': 'Generally well-behaved, supports latest WiFi standards', 'icon': '🍎'},
    'Ring': {'type': 'security_camera', 'concern': 'Security camera - high bandwidth, consider QoS', 'icon': '📹'},
    ...
    'Sonos': {'type': 'streaming', 'concern': 'Audio streaming - sensitive to network latency', 'icon': '🔊'},
    'Roku': {'type': 'streaming', 'concern': 'Streaming device - benefits from QoS priority', 'icon': '📺'},
    'Samsung': {'type': 'consumer', 'concern': 'Check if smart TV - may need firmware update', 'icon': '📱'},
    
    # Security/Cameras
    'Wyze': {'type': 'security_camera', 'concern': 'Security camera - isolate from main network', 'icon': '📹'},
    'Arlo': {'type': 'security_camera', 'concern': 'Security camera - high upload bandwidth', 'icon': '📹'},
    'Ubiquiti': {'type': 'network_equipment', 'concern': 'Network equipment - should be on management VLAN', 'icon': '📡'},
    
    # Gaming
    'Sony': {'type': 'gaming', 'concern': 'Gaming console - benefits from low latency', 'icon': '🎮'},
    'Microsoft': {'type': 'gaming', 'concern': 'Gaming/PC - may need port forwarding', 'icon': '🎮'},
    ...
    # Smart Home Hubs
    'Lutron': {'type': 'smart_home_hub', 'concern': 'Smart home controller - critical for automation', 'icon': '🏡'},
    'Control4': {'type': 'smart_home_hub', 'concern': 'Home automation - should be on stable network', 'icon': '🏡'},
...
def analyze_manufacturers(clients):
    ...
    return {
        ...
        'total_categorized': sum(len(devices) for cat, devices in device_categories.items() if cat != 'unknown'),
...
def generate_manufacturer_insights_html(manufacturer_analysis):
    ...
    category_html = ""
    category_names = {
        'iot': ('🏠 IoT & Smart Home', 'Smart home devices, voice assistants, and connected appliances'),
        'security_camera': ('📹 Security Cameras', 'IP cameras requiring continuous bandwidth and upload capacity'),
        ...
        'network_equipment': ('📡 Network Equipment', 'Infrastructure devices requiring management access'),
    ...
    if recommendations:
        for rec in recommendations:
            color = '#ef4444' if rec['priority'] == 'high' else '#f59e0b' if rec['priority'] == 'medium' else '#3b82f6'
