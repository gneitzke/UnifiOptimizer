...
def generate_api_error_warning_html(api_errors):
    ...
    if failed_endpoints:
        ...
        for endpoint in failed_endpoints[:10]:  # Show max 10
            ...
            friendly_name = endpoint.replace('s/default/', '').replace('s/home/', '').replace('stat/', '')
    ...
    error_types_html = ""
    if errors_by_type:
        error_types_html = "<div style='margin-top: 15px; padding: 10px; background: white; border-radius: 4px;'>"
...
def generate_ap_overview_html(ap_analysis, mesh_aps):
    ...
    for ap in aps:
        ...
        if ng_power or ng_width:
            ng_details += f"<br><span style='font-size: 0.85em; color: #666;'>{ng_power}, {ng_width}</span>"
        
        # Build 5GHz details  
        na_details = f"{na_channel}" if na_channel else "-"
        if na_power or na_width:
            na_details += f"<br><span style='font-size: 0.85em; color: #666;'>{na_power}, {na_width}</span>"
