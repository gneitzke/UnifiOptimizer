...
class ClientHealthAnalyzer:
    ...
    def _calculate_health_scores(self):
        ...
        for client in self.clients:
            ...
            disconnect_penalty = len([e for e in self.disconnection_events 
                                     if e.get('user', '') == mac or e.get('client_mac', '') == mac]) * 5
            
            # Penalty for excessive roaming
            roam_penalty = len([e for e in self.roaming_events 
                               if e.get('user', '') == mac or e.get('client_mac', '') == mac]) * 2
...
def display_client_health_report(analysis):
    ...
    console.print("\n")
    console.print(Panel("[bold cyan]Client Health Diagnostic Report[/bold cyan]", 
                       style="cyan"))
    ...
    if len(weak) > analysis['total_clients'] * 0.2:
        recommendations.append("• Over 20% of clients have weak signal - consider adjusting AP placement or power levels")
    
    if prone:
        recommendations.append(f"• {len(prone)} clients experiencing frequent disconnections - investigate interference or AP issues")
    
    critical_count = len(analysis['by_signal_quality']['critical'])
    if critical_count > 0:
        recommendations.append(f"• {critical_count} clients in critical signal range (< -80 dBm) - immediate attention needed")
