...
- **Controller URL**: e.g., `https://192.168.1.1` or `https://unifi.yourdomain.com`    │                                                             │
...
- **Best for**: Mobile sharing, email attachments  • 3 wired uplink APs- **Automatic HTTPS prefix**: Just enter `192.168.1.1` and it becomes `https://192.168.1.1`
...
```bash---- First run: you'll be prompted for host (e.g. `192.168.1.1` - the `https://` prefix is automatically added), site (`default`), auth method (`user` or `key`), and mode (`deep` is a good start).
...
python3 core/optimize_network.py analyze \
  ...
  --host https://192.168.1.1 \### Latest Version - v2.0 "Network Intelligence"
...
python3 core/optimize_network.py apply --dry-run \- ✅ **2FA Support**: Full two-factor authentication support for secure UniFi accounts
  ...
  --host https://192.168.1.1 \- ✅ **Smart URL Handling**: Automatic `https://` prefix addition and `http://` to `https://` conversion
